...
/**
 * Always-on poller for ~1-minute cadence (faster than WorkManager's 15-min floor) that keeps working
 * even in Doze. Runs as a dataSync foreground service while "실시간 감시" is enabled. Android requires
 * an FGS to show a notification; we keep it on a min-importance channel (no status-bar icon).
 */
